...
const Job = ({ user_id, company, jobType, position, status, jobLocation, createdAt }) => {
    const date = moment(createdAt).format("MMM Do, YYYY");
    const dispatch = useDispatch();

    const handleDeleteJob = () => {
        dispatch(deleteJob(user_id));
    };

    const handleEditJob = () => {
        dispatch(setEditJob({ editJobId: user_id, company, jobLocation, jobType, position, status }));
        dispatch(updateSelectedId({ currId: "3" }));
    };

    return (
        <div id={user_id} className="card card-compact w-full bg-base-100 shadow-xl mt-3 border">
            ...
            <div className="card-body grid  grid-cols-1 sm:grid-cols-2 bg-gray-100">
                ...
                <div>
                    ...
                    <p
                        className={`rounded-lg w-1/2 mx-4  p-1 text-center ${
                            status === "pending"
            ...
            <div className="card-actions justify-start mx-4 mb-3 bg-gray-100">
                <Link to="/add-job">
                    <button className="btn btn-primary px-2 btn-sm" onClick={handleEditJob}>
                        Edit
                    </button>
                </Link>
                <button
                    name={user_id}
                    className="btn-outline btn-error border rounded-md px-2 btn-sm"
